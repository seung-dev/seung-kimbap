...
export const LoginPage: React.FC<LoginProps> = ({
	...
	const content = (
		<div {...contentProps}>
			<h1 style={{ textAlign: "center" }}>{translate("pages.login.title", "Sign in to your account")}</h1>
			{renderProviders()}
			<hr />
			<form
				onSubmit={(e) => {
			...
			>
				...
				>
					...
					/>
					<label htmlFor="password-input">{translate("pages.login.fields.password", "Password")}</label>
					<input
						id="password-input"
						type="password"
						name="password"
					...
					<br />
					{forgotPasswordLink ??
						renderLink("/forgot-password", translate("pages.login.buttons.forgotPassword", "Forgot password?"))}
					<input type="submit" value={translate("pages.login.signin", "Sign in")} />
					{registerLink ?? (
						<span>
