...
export function Authenticated(props: LegacyAuthenticatedProps): JSX.Element | null;
...
export function Authenticated({
	...
	if (state.current.status === "settled") {
		...
		if (isAuthenticated) {
			 ...
			 */
			const pathname = `${isLegacyRouter ? legacyLocation?.pathname : parsed.pathname}`.replace(/(\?.*|#.*)$/, "");
