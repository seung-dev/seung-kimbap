...
export const ListButton: React.FC<ListButtonProps> = ({
	...
	const hideIfUnauthorized = accessControl?.hideIfUnauthorized ?? accessControlContext.options.buttons.hideIfUnauthorized;
	...
	const { resource, identifier } = useResource(resourceNameFromProps ?? resourceNameOrRouteName);
		...
		>
			...
			>
				{hideText ? (
					...
					children ??
					translate(
						`${identifier ?? resourceNameFromProps ?? resourceNameOrRouteName}.titles.list`,
						getUserFriendlyName(
							resource?.meta?.label ??
								resource?.label ??
								identifier ??
								pickNotDeprecated(resourceNameFromProps, resourceNameOrRouteName),
							"plural",
						),
					)
				)}
